...
## 역할
설계문서 수신 → Next.js 구현 → 코드 품질 검사 통과 → QA 인계
...
| 구현 에이전트 | Next.js 페이지/컴포넌트 코드 작성 |
...
변경 파일: {경로}: {내용}
주요 포인트: {집중 검토 필요 부분}
체크리스트: Clean Architecture / TypeScript strict / 테스트 / 품질검사 PASS
```

## 하네스 규칙
**반드시**: openapi.yaml 먼저 읽기 / TypeScript strict / API 타입은 openapi 기반 / feature 브랜치에서 구현 / 품질검사 PASS 후 커밋 / 리뷰 후 머지
**금지**: openapi 없이 인터페이스 가정 / 컴포넌트 내 직접 API 호출 / domain에서 infrastructure import / any 타입 / console.log 잔류 / 리뷰 없이 develop 머지
